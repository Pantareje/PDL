\begin{tabbing}
    ...
    \>2. P → STATEMENT P\\
    \>3. P → $eof$\\
    \>4. FUNCTION → \>function \textbf{\{if (TSL != NULL) then error(-)\}} FUNTYPE\\
    ...
    \>11. FUNATTRIBUTES → \>VARTYPE $id$ \textbf{\{InsertaTipoTS(id.pos, VARTYPE.tipo);}\\
    ...
    \>12. NEXTATTRIBUTE → , VARTYPE $id$ \textbf{\{InsertaTipoTS(id.pos, VARTYPE.tipo);}\\
    \>                    \>        \>\textbf{InsertaDespTS(id.pos, DespL);}\\
    \>                    \>        \>\textbf{DespL := DespL + VARTYPE.tipo\}}\\
    ...
    \>                    \> \textbf{STATEMENT.tipoRet := ATOMSTATEMENT.tipoRet\}}\\
    \>20. ATOMSTATEMENT → \textbf{\{declararImplicito := true\}} $id$ IDACT ;\\
    ...
    \>21. ATOMSTATEMENT → $output$ EXP1 ; \textbf{\{ATOMSTATEMENT.tipo := }\\
    \>                    \> \textbf{if (EXP1.tipo $\in$ \{int, str\}) then}\\
    \>                    \> \> \textbf{tipo\_ok}\\
    \>                    \> \textbf{else tipo\_error}\\
    \>                    \> \textbf{ATOMSTATEMENT.tipoRet := void\}}\\
    \>22. ATOMSTATEMENT → $input$ $id$ ; \textbf{\{ATOMSTATEMENT.tipo := }\\
    ...
    \>26. FORACT → $id$ ASS EXP1 \textbf{\{FORACT.tipo :=}\\
    ...
    \>30. CALLPARAM → EXP1 NEXTPARAM \textbf{\{if (EXP1.tipo != tipo\_error}\\
    ...
    \>32. NEXTPARAM → , EXP1 NEXTPARAM$_1$ \textbf{\{if (EXP1.tipo != tipo\_error}\\
